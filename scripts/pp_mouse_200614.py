...
def log2aveFC(X,Y,gene_names,AnnData) :
    ...
    fc=pd.DataFrame({'Gene':AnnData.var_names[g_idx],
                     'log2FC':np.log2(X[:,g_idx].mean(axis=0) - Y[:,g_idx].mean(axis=0))})
    return fc
...
if True :
    ...
    if True :
        ...
        fig.savefig(os.path.join(pfp,'batchEffect_mouse_200614.pdf'),dpi=300,bbox_inches='tight')
        del tdata
...
#         adata.layers['imputed_bbknn']=sparse.csr_matrix(magic_op.transform(adata.X,genes='all_genes')) # causes memory spike
    ...
    # load data objects
    start=time.time()
    adata = sc.read_h5ad(os.path.join(pdfp,'mouse_200614.h5ad'))
...
if False :
    print('Recalculating louvain clusters')
    ...
    if True :
        ...
        adata.write(os.path.join(pdfp,'mouse_200614.h5ad'))
        print('\n... saved @'+datetime.datetime.now().strftime('%y%m%d.%H:%M:%S'))
        print('... took {:.2f}-s to save'.format(time.time()-start))
...
if True :
    ...
    if False :
        ...
        scprep.plot.marker_plot(adata.X,adata.obs[cluster], # imputed here?
                                ...
                                cmap='bwr',
                                ax = ax)
        fig.savefig(os.path.join(pfp,'mouse_200614_scprepdot.pdf'),bbox_inches='tight')
        ...
        for k,v in markers.items() :
            for g in v :
                ...
                scprep.plot.scatter2d(adata.obsm['X_umap'],
                                      ...
                                      s = 0.2,
                                      ax = ax)
        fig.savefig(os.path.join(pfp,'mouse_200614_markersUMAP.png'),dpi=600, bbox_inches='tight')
...
if False :
    ...
    sc.plotting.rank_genes_groups_matrixplot(adata,n_genes=10,cmap='magma',standard_scale='var',save = '_mouse_200614.png')
    sc.pl.rank_genes_groups(adata,n_genes=50,fontsize=6,save='_mouse_200614.pdf')
    ...
    if False :
        ...
        dt.to_csv(os.path.join(pfp,'mouse_200614_overlapTop500.csv'))
        del dt
...
if False :
    ...
    for t in ['5wk','12wk','18wk','24wk','30wk'] :
        ...
        print('--------\n')
        dge = pd.DataFrame()
        for i in ['12']:#tdata.obs[cluster].unique() :
            start = time.time()
            print('\n{}, WT vs SCA1'.format(i))
            print('----')
...
if True :
    ...
    scprep.plot.scatter2d(adata.obsm['X_umap'],
                          c=adata.obs['batch'].astype(int).to_numpy(),
                          cmap=cmap_sample,
                          ticks=None,
                          label_prefix='UMAP',
                          legend=False,
    ...
    fig.savefig(os.path.join(pfp,'embeds_mouse_200614.pdf'),dpi=300, bbox_inches='tight')

...
set1 = [3,5,10,20]
set2 = [30, 45, 100]
for i, t in enumerate(set1):
    for j, k in enumerate(set2):
        ...
        for c in ['Purkinje cell']:
            X = wt[((wt.obs[group]==c)), :].layers['imputed']
            X_mut = mut[((mut.obs[group]==c)), :].layers['imputed']
        ...
        print('  ... through {:.1f}-% in {:.1f}-min'.format(100*(i+j+2)/(len(set1)*len(set2)), (time.time() - grand_t)/60))

...
if True :
    ...
    for t in ['5wk','12wk','18wk','24wk','30wk'] :
        ...
        for i in tdata.obs[cluster].unique() :
            ...
            X = tdata.X[np.where((tdata.obs[cluster]==i) & (tdata.obs['genotype']=='WT'))[0],:]
            Y = tdata.X[np.where((tdata.obs[cluster]==i) & (tdata.obs['genotype']=='SCA1'))[0],:]
        ...
        dge.to_csv(os.path.join(pfp,'dge_{}_{}_WTup.csv'.format(t,fname)),index=False)
        ...
        if True :
            ...
            for i in tdata.obs[cluster].unique() :
                ...
                print(dt.iloc[0:10,:])
                print('... computed in {:.2f}-s'.format(time.time()-start))
            dge.to_csv(os.path.join(pfp,'dge_{}_{}_WTdown.csv'.format(t,fname)),index=False)
